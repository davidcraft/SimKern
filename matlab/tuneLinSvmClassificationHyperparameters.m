...
for i_c = 1:numel(cValues)
    ...
    [linSvmModel{i_c}] = svmtrain(trainData.classes, trainData.dummycodedFeatures, inputString);
    % testSvm on validation data
    [predictions{i_c}] = svmpredict(zeros(size(validationData.classes,1),size(validationData.classes,2)),validationData.dummycodedFeatures,linSvmModel{i_c},'-q');
