...
f = '..\SimKernModels\Radiation\DataReadyForML\Sim1SimilarityMatrixfinalHIGHERQUALITY.csv';
sm =  csvread(f);
...
timeStamp = strrep(timeStamp,'-','_');
timeStamp = strrep(timeStamp,' ','_');
% pathToMatFile = fullfile(pathToDataFolder,['radiation_' timeStamp]);
pathToMatFile = fullfile(pathToDataFolder,'radiation');
save(pathToMatFile)

% pathToMatFileLight = fullfile(pathToDataFolder,['radiation_light_' timeStamp]);
pathToMatFileLight = fullfile(pathToDataFolder,'radiation_light');
