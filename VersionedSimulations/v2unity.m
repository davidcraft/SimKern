...
% set single = true for single compartment, false for double compartment
    ...
    if single
        ...
        xd(P_P53Nuc) = barkS + barkdph1 * x(P_WIP1Nuc) * ((x(P_P53NucPhos)/(barKdph1+x(P_P53NucPhos)))) ...
            - bark1 * x(P_MDM2Nuc) * (x(P_P53Nuc)/(barK1+x(P_P53Nuc))) ...
                    -bark3 * x(P_ATMNucPhos) * (x(P_P53Nuc)/(barKatm+x(P_P53Nuc))) - delta_p*(x(P_P53Nuc));
        % Mdm2
        xd(P_MDM2Nuc) = barktm * x(M_MDM2Nuc) - x(P_MDM2Nuc);
        % Mdm2 mRNA
        xd(M_MDM2Nuc) = barkSm + barkSpm * (x(P_P53NucPhos)^4/(barKSpm^4+x(P_P53NucPhos)^4)) - delta_mrna * x(M_MDM2Nuc) ...
                    - barktm * x(M_MDM2Nuc);
        % p53_p
        ...
        %Wip1
        xd(P_WIP1Nuc) = barktw * x(M_WIP1Nuc) - delta_w * x(P_WIP1Nuc); 
        %Wip1 mRNA
        xd(M_WIP1Nuc) = barkSw + barkSpw * (x(P_P53NucPhos)^4/(barKSpw^4+x(P_P53NucPhos)^4)) - delta_wrna * x(M_WIP1Nuc) ...
                    - barktw * x(M_WIP1Nuc);
        % Atm_p
