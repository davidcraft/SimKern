...
function xd=f(t,x)
    ...
    global single;
    if single
    s_s = 1; s_m = 0; %these represent the single and multi compartment switch values
    else
    s_s = 0; s_m = 1;
    end
    variableDefinition3;
    ...
    % equations for single AND multi compartment model
    % p53
    xd(P_P53Nuc) = s_s*barkS + barkdph1 * x(P_WIP1Nuc) * (x(P_P53NucPhos) / (barKdph1 + x(P_P53NucPhos)))- bark1 * x(P_MDM2Nuc) * (x(P_P53Nuc) / (barK1+x(P_P53Nuc))) ...
                    - bark3 * x(P_ATMNucPhos) * (x(P_P53Nuc) / (barKatm + x(P_P53Nuc))) - s_m*barpp * Vr * (x(P_P53Nuc) - x(P_P53Cyto)) - s_s*deltap*(x(P_P53Nuc));          
    % Mdm2
    xd(P_MDM2Nuc) = s_s*barktm - s_m*barpm * Vr * (x(P_MDM2Nuc) - x(P_MDM2Cyto)) - bardeltam * x(P_MDM2Nuc);
    % Mdm2 mRNA
    xd(M_MDM2Nuc) = barkSm + barkSpm * (x(P_P53NucPhos)^4 / (barKSpm^4 + x(P_P53NucPhos)^4)) - x(M_MDM2Nuc)*(s_m*barpmrna * Vr  ...
                    + bardeltamrna  + s_s*barktm);
    % p53_p
    ...
    %Wip1
    xd(P_WIP1Nuc) = s_s*barktw * x(M_WIP1Nuc) + s_m*barpw * Vr * x(P_WIP1Cyto) - bardeltaw * x(P_WIP1Nuc);
    %Wip1 mRNA
    xd(M_WIP1Nuc) = barkSw + barkSpw * (x(P_P53NucPhos)^4/(barKSpw^4+x(P_P53NucPhos)^4)) - x(M_WIP1Nuc)*(s_s*deltawrna +  ...
                s_s*barktw + s_m*barpwrna * Vr);
    % Atm_p
